...
namespace WeightCenterDesignAndEstimateSoft.Task
{
    public partial class CoreEnvelopeCutForm : Form
    {
        ...
        private void buttonImportFromCutData_Click(object sender, EventArgs e)
        {
            ...
            if (coreEnvelopeCut != null && coreEnvelopeCut != null && coreEnvelopeCut.lstBasicCoreEnvelope.Count > 0)
            {
                if (data.lstBasicCoreEnvelope == null)
                {
                    data.lstBasicCoreEnvelope = new List<CorePointData>();
        ...
        private void DiscreteCore()
        {
            ...
            if (!CreateCircularPtNums(discreteset.nCircularPtCount, data.lstBasicCoreEnvelope, out CircularPt0))
            {
        ...
        private bool CreateCircularPtNums(int totalnum, List<CorePointData> ptlst, out List<CorePointData> outptlst)
        {
            outptlst = null;
            if (ptlst.Count < 3)
            {
            ...
            else
            {
                ...
                for (int i = 0; i < ptlst.Count; ++i)
                {
                    ...
                    double flenth = Math.Sqrt(dx * dx + dy * dy);
                    lens.Add(flenth);
                    totallen += flenth;
                }
