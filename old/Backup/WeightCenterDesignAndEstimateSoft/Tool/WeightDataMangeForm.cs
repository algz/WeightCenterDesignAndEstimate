...
namespace WeightCenterDesignAndEstimateSoft.Tool
{
    public partial class WeightDataMangeForm : Form
    {
        ...
        {
            List<TypeWeightData> lstWeightData = null;
            ...
            XmlNodeList lstNodeDB = doc.SelectNodes("structer");

            if (lstNodeDB != null && lstNodeDB.Count > 0)
            {
                List<TypeWeightData> lstTypeWeightData = new List<TypeWeightData>();

                string dbId = lstNodeDB[0].Attributes["id"].Value;
                lstClassNode = doc.GetElementsByTagName("inClass");

                List<string> lstID = new List<string>();
                Dictionary<string, string> dicID = new Dictionary<string, string>();
                foreach (XmlNode typeNode in lstClassNode)
                {
                    XmlAttribute att = typeNode.Attributes["pid"];
                    //型号ID
                    if (att.Value == dbId && typeNode.Attributes["weightType"].Value == "folder")
                    {
                        lstID.Add(typeNode.Attributes["classId"].Value);
                    }
                }

                foreach (string str in lstID)
                {
                    string strDataId = string.Empty;
                    ...
                    foreach (XmlNode typeNode in lstClassNode)
                    {
                        XmlAttribute att = typeNode.Attributes["pid"];
                        //重量数据ID
                        if (str == att.Value && typeNode.Attributes["weightType"].Value == "object")
                        {
                            strDataId = typeNode.Attributes["classId"].Value;
                        }
                        //分类方法ID
                        if (str == att.Value && typeNode.Attributes["weightType"].Value == "folder")
                        {
                            strSortId = typeNode.Attributes["classId"].Value + "," + typeNode.Attributes["name"].Value;
                        }
                        if (strDataId != string.Empty && strSortId != string.Empty)
                        {
                            dicID.Add(strDataId, strSortId);
                            break;
                        }
                    }
                }
                ...
                lstNodeInstance = doc.GetElementsByTagName("instance");

                int Count = 0;
                if (lstNodeInstance != null && lstNodeInstance.Count > 0)
                {
                    lstWeightData = new List<TypeWeightData>();
                    foreach (KeyValuePair<string, string> keyPair in dicID)
                    {
                        TypeWeightData weightData = new TypeWeightData();
                        weightData.Id = bllTypeWeight.GetMaxId() + Count;
                        weightData.MainSystem_Name = keyPair.Value.Split(',')[0];
                        Count++;
                        foreach (XmlNode typeNode in lstNodeInstance)
                        {
                            XmlAttribute att = typeNode.Attributes["pid"];
                            if (keyPair.Key == att.Value)
                            {
                                XmlNodeList lstNodeProperty = typeNode.ChildNodes;
                                if (lstNodeProperty != null && lstNodeProperty.Count > 0)
                                {
                                    foreach (XmlNode propertyNode in lstNodeProperty)
                                    {
                                        XmlAttribute attProperty = propertyNode.Attributes["name"];
                                        if (attProperty.Value == "机型名称")
                                        {
                                            weightData.Helicopter_Name = propertyNode.Attributes["value"].Value;
                                        }
                                        if (attProperty.Value == "最后修改时间")
                                        {
                                            weightData.Last_ModifyTime = propertyNode.Attributes["value"].Value;
                                        }
                                        if (attProperty.Value == "机型国籍")
                                        {
                                            weightData.Helicoter_Country = propertyNode.Attributes["value"].Value;
                                        }
                                        if (attProperty.Value == "设计起飞重量")
                                        {
                                            weightData.DesignTaking_Weight = propertyNode.Attributes["value"].Value == null ? 0 : Convert.ToDouble(propertyNode.Attributes["value"].Value);
                                        }
                                        if (attProperty.Value == "最大起飞重量")
                                        {
                                            weightData.MaxTaking_Weight = propertyNode.Attributes["value"].Value == null ? 0 : Convert.ToDouble(propertyNode.Attributes["value"].Value);
                                        }
                                        if (attProperty.Value == "空机重量")
                                        {
                                            weightData.EmptyWeight = propertyNode.Attributes["value"].Value == null ? 0 : Convert.ToDouble(propertyNode.Attributes["value"].Value);
                                        }
                                        if (attProperty.Value == "机型类型")
                                        {
                                            weightData.Helicopter_Type = propertyNode.Attributes["value"].Value == null ? string.Empty : propertyNode.Attributes["value"].Value;
                                        }
                                    }
                                }
                                lstWeightData.Add(weightData);
                ...
                #region 重量数据
                List<WeightSortData> lstWeightSort = new List<WeightSortData>();
                foreach (KeyValuePair<string, string> keyPair in dicID)
                {
                    string[] strArray = keyPair.Value.Split(',');
                    string strRootID = GetClassID(strArray[0]);
                    string strRemark = string.Empty;

                    WeightSortData sortData = new WeightSortData();
                    sortData.lstWeightData = new List<WeightData>();
                    sortData.sortName = strArray[1];
                    sortData.strRemark = strArray[0];

                    foreach (XmlNode typeNode in lstClassNode)
                    {
                        string strID = typeNode.Attributes["classId"].Value;

                        XmlAttribute att = typeNode.Attributes["pid"];

                        if (att.Value == strRootID && typeNode.Attributes["weightType"].Value == "folder")
                        {
                            List<string> lstClassID = GetListClassID(strID);
                            foreach (string str in lstClassID)
                            {
                                //根节点
                                string strRoot = GetClassRemark(string.Empty, strRootID, string.Empty);
                                strRemark = GetClassRemark(strRootID, strID, strRoot);
                                strRemark = GetClassRemark(strID, str, strRemark);
                                WeightData data = GetWeightData(str, strRemark);
                                sortData.lstWeightData.Add(data);
                            }
                        }
                        else if (att.Value == strRootID && typeNode.Attributes["weightType"].Value == "object")
                        {
                            //根节点
                            string strRoot = GetClassRemark(string.Empty, strRootID, string.Empty);
                            strRemark = GetClassRemark(strRootID, strID, strRoot);
                            WeightData data = GetWeightData(strID, strRemark);
                            sortData.lstWeightData.Add(data);
                        }
                    }
                ...
                //插入重量数据
                if (lstWeightData != null && lstWeightData.Count > 0 && lstWeightSort != null && lstWeightSort.Count > 0)
                {
                    foreach (TypeWeightData weightData in lstWeightData)
                    {
                        foreach (WeightSortData sortData in lstWeightSort)
                        {
                            if (weightData.MainSystem_Name == sortData.strRemark)
                            {
                                weightData.MainSystem_Name = GetMainSystemWeight(sortData);
                            }
                        }
                    }
                }
        ...
        private string GetClassID(string strParentID)
        {
            string strID = string.Empty;
            foreach (XmlNode typeNode in lstClassNode)
            {
                XmlAttribute att = typeNode.Attributes["pid"];
                if (att.Value == strParentID)
                {
                    strID = typeNode.Attributes["classId"].Value;
                    break;
                }
            }
            return strID;
        ...
        private List<string> GetListClassID(string strParentID)
        {
            List<string> lstClsassID = new List<string>();
            string strID = string.Empty;
            ...
            foreach (XmlNode typeNode in lstClassNode)
            {
                ...
                if (att.Value == strParentID)
                {
                    strID = typeNode.Attributes["classId"].Value;
                    //修改
                    if (typeNode.Attributes["weightType"].Value == "object")
                    {
                        lstClsassID.Add(strID);
                    }
                    else if (typeNode.Attributes["weightType"].Value == "folder")
                    {
                        GetListClassID(strID);
                    }
        ...
        private string GetClassRemark(string strParentID, string strID, string strRootRemark)
        {
            string strRemark = string.Empty;
            foreach (XmlNode typeNode in lstClassNode)
            {
                ...
                if (typeNode.Attributes["classId"].Value == strID && strRootRemark == string.Empty)
                {
                    strRemark = typeNode.Attributes["name"].Value; ;
                }
                else
                {
                    if (att.Value == strParentID)
                    {
                        if (typeNode.Attributes["classId"].Value == strID && typeNode.Attributes["weightType"].Value == "folder")
                        {
                            strRemark = strRootRemark + "\\" + typeNode.Attributes["name"].Value; ;
                        }
                        else if (typeNode.Attributes["classId"].Value == strID && typeNode.Attributes["weightType"].Value == "object")
                        {
                            strRemark += strRootRemark + "\\" + typeNode.Attributes["name"].Value;
                            return strRemark;
                        }
        ...
        /// <returns></returns>
        private WeightData GetWeightData(string strParentID, string strRemark)
        {
            WeightData data = null;
            foreach (XmlNode typeNode in lstNodeInstance)
            {
                XmlAttribute att = typeNode.Attributes["pid"];
                if (strParentID == att.Value)
                {
                    XmlNodeList lstNodeProperty = typeNode.ChildNodes;
                    if (lstNodeProperty != null && lstNodeProperty.Count > 0)
                    {
                        ...
                        {
                            XmlAttribute attProperty = propertyNode.Attributes["name"];
                            if (attProperty.Value == "重量名称")
                            {
                                data.weightName = propertyNode.Attributes["value"].Value;
                            }
                            if (attProperty.Value == "ID")
                            {
                                data.nID = Convert.ToInt32(propertyNode.Attributes["value"].Value);
                            }
                            if (attProperty.Value == "重量数值")
                            {
                                data.weightValue = propertyNode.Attributes["value"].Value == null ? 0 : Convert.ToDouble(propertyNode.Attributes["value"].Value);
                            }
                            if (attProperty.Value == "重量父节点")
                            {
                                data.nParentID = Convert.ToInt32(propertyNode.Attributes["value"].Value);
                            }
                        }
                    }
                }
        ...
        ///// 从基础库获取数据
        ///// </summary>
        //public void BasicDBGetData()
        //{
        //    //调用接口生成文件
        //    WebReference.xmlservice a = new WeightCenterDesignAndEstimateSoft.WebReference.xmlservice();
        //    string strFileName = a.pareseXMlFile().@return;

        //    //服务器文件地址c:/fileserver+年(2013)+月日(1225)+小时(15)
        //    //读取文件存放在BasicData目录下
        ...
        //    string path = @"fileserver\" + strDate.Split('-')[0] + "\\"
        //        + strDate.Split('-')[1] + strDate.Split('-')[2] + "\\" + strTime.Split(':')[0] + "\\";

        //    XmlDocument doc = new XmlDocument();
        //    //获得配置文件的全路径
        //    string strConfigFile = System.AppDomain.CurrentDomain.SetupInformation.ConfigurationFile;
        ...
        //    string strURL = @"\\" + strIp + "\\" + path + strFileName;

        //    List<string> lstContent = new List<string>();
        //    CommonFunction.mReadFileToListStringBasic(strURL, lstContent);
        ...
        //    }
        //    string strDestFilePath = strFolder + "\\" + strFileName + ".xml";
        //    CommonFunction.mWriteListStringToFileBasic(strDestFilePath, lstContent);
        ...
        //        List<TypeWeightData> lstWeightData = GetListTypeWeightData(strDestFilePath);
        //        //DB-Data
        ...
        /// 从基础库获取数据
        /// </summary>
        public void BasicDBGetData(Model.BasicDBSetting dbSetting)
        {
            try
            {
                //测试调用接口生成文件
                WebReference.xmlservice xmlservice = new WeightCenterDesignAndEstimateSoft.WebReference.xmlservice();
                string strFileName = xmlservice.pareseXMlFile().@return;
                xmlservice.Timeout = 3000;
                ...
                string strDate = strDateTime.Split(' ')[0];
                string strTime = strDateTime.Split(' ')[1];
                string path = "\\" + dbSetting.strFolder + "\\" + strDate.Split('-')[0] + "\\"
                 + strDate.Split('-')[1] + strDate.Split('-')[2] + "\\" + strTime.Split(':')[0] + "\\";

                string strURL = @"\\" + dbSetting.strServer + "\\" + path + strFileName;

                List<string> lstContent = new List<string>();

                bool IsRead = CommonFunction.mReadFileToListStringBasic(strURL, lstContent);
                if (IsRead == false)
                {
                    MessageBox.Show("从基础数据库同步数据失败");
                    return;
                }
                XLog.Write("从基础数据库同步数据进行中");
        ...
        private void btnFromBasicToDB_Click(object sender, EventArgs e)
        {
            try
            {
                BasicDBSetForm form = new BasicDBSetForm(this);
                form.ShowDialog();
            }
            catch (Exception ex)
            {
